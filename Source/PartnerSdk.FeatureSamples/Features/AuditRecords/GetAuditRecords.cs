// <copyright file="GetAuditRecords.cs" company="Microsoft Corporation">
...
namespace Microsoft.Store.PartnerCenter.FeatureSamples.Features.Invoices
{
    ...
    /// <summary>
    /// Showcases get audit records.
    /// </summary>
    internal class GetAuditRecords : IUnitOfWork
    {
        /// <summary>
        ...
        public void Execute(IAggregatePartner partnerOperations, IDictionary<string, object> state)
        {
            ...
            var collection = scopedPartnerOperations.AuditRecords.Query(startDate.Date, query: QueryFactory.Instance.BuildIndexedQuery(10));
            ...
            while (enumerator.HasValue)
            {
                ...
                foreach (var record in enumerator.Current.Items)
                {
                    ...
                    if (record.CustomizedData != null && record.CustomizedData.Any())
                    {
                        Console.Out.WriteLine("Customized Data:");
                        record.CustomizedData.ToList().ForEach(x => Console.WriteLine("                        {0} : {1}", x.Key, x.Value));
                    }
